...
f64 process_followup(Process::command_list in_cmd_type, std::string next_arg) {
	...
	switch (in_cmd_type) {
	case Process::command_list::ROLL_DICE:  // this might fail with junk data, not thoroughly tested
	{
		int num1 = 0;
		...
		if (num1 > 0 && num2 > 0) {
			...
			{
				results[i] = Random::NTKR(1, num2);
				total_temp += results[i];
			}
			Display::RollResult(num1, num2, results);
			return_value = total_temp;
		}
	}
	break;
